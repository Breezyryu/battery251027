...
------------------------------------------------------------------------------------------------------------------------
210923 
    refactoring - 변수 이름 변경, 함수 간결화, 파일경로 선택 오류 개선
211207 
    장수명 알고리즘 개선 (갯수 제한 해제)
220127 
    저장 그래프 중 온도 30, 40 추가
220711 
    Toyo 방전에 CV 방전 용량 추가
220819 
    writer 외 global 변수 제거
230329 
    PNE 급속 충전 Step 갯수 제한 해제 (동적변수)
230329 
    그래프 함수 refactoring
230502 
    충방전기 현황 추가
230807 
    간이 경로 추가, 폴더 없을 때 수정
240201 
    Samsung One font로 변경
240101
    PNE 현황에 온도, 사이클, 문자열 검색 기능 부분 추가
240201
    PNE 패턴 수정을 위한 기능 추가
240301
    PNE 패턴 수정 시 특정 패턴 선택하여 변경가능하게 변경
    PNE 현황판에 현재 걸려있는 셀의 전압 확인 및 네트워크 저장 경로 추가
240315
    신뢰성데이터 확인 시 파일에 nasca 걸려있을 경우 오류 발생 수정
    신뢰성데이터 방전 2step의 경우 방전 부분 합해서 계산하는 부분 추가
    PNE 현황판에 현재 걸려있는 셀의 경로에서 채널 번호 삭제
240322
    폰트 기본 폰트(malgun gothic)으로 변경(별도 폰트 설치 불편함 문제)
    전체 패턴 변경 부분 삭제 (의도치 않은 패턴 변경 문제 방지)
240326
    사이클 중 RSS 저항 옵션 설정 시 오류 발생 부분 수정
240401
    Step Charge Profile 검토 시 오류 수정
240408
    방전 Profile에서 step 방전도 확인 가능하게 기능 추가
240411
    Step 방전 Profile 오류 수정
240507
    패턴 수정 부분에 folder 이름 등 추가
    패턴 수정 부분 여러 패턴 동시 변경 가능하게 변경
240528
    패턴 수정 부분에서 전류가 내림되는 부분을 올림으로 변경
240716
    충방전기 추가에 따른 UI 및 드라이브 추가 변경 (류성택님 의견 - 콤보박스형식)
240726
    스텝 형식 중 -로도 스텝 넣도록 변경
    없는 스텝 넣을 경우 꺼지는 오류 수정
240731
    HPPC 데이터 확인 부분 추가
    오류 메세지 부분 추가
240731
    pne cycle에 맞춰 Profile 있는 파일 찾는 부분 추가
240808
    지정 용량에 소수점 넣을 수 있게 수정
    cyclepath 파일에 tab으로 cyclename을 넣을 경우 지정 이름을 넣어줄수 있게 변경
    이름 넣었을 경우 범례를 사이클과 저항 부분에 추가
241007
    HPPC/GITT(연속 profile 기준)
    DCIR(신규 패턴 기준) 기능 추가
    cycle 결과에 cycle name 기능 추가
    DCIR 부분 0일 경우 자동으로 산정되게 변경
    통합사이클로 맨 뒤에 승인 사이클 추가 가능
241016
    개별 사이클 범례, 색상 관련 수정
241107
    수명 예측 tab 추가 - 장수명 반영
...
250206
    그래프 수정
    연결 그래프를 별도 datapath로 만들어서 여러개 동시 그래프 그릴수 있는 버튼 추가
250306
    EU 수명 예측을 위한 raw file 형식 변경에 따른 별도 알고리즘 추가 및 수정하였습니다.
    ECT 알고리즘 확인을 위한 ECT log 확인 부분 오류 수정하였습니다.
250415
    그래프를 프로그램 안으로 삽입 (해상도 변화)
    데이터연결 시 output 문제 수정
    신뢰성 hybrid 수명 뒷부분 합산 안되는 문제 수정
250527
    PNE21 충방전기 추가
    EU 수명 예측 추가
    글꼴 맑은 고딕으로 원복
    전체 창 모드 기준으로 변경 (1920 * 1080)
250703
    PNE21 coin cell 용량 반영 안 되는 문제 (4-6mAh와 같이 소수점 용량 반영)
    PNE21 패턴 수정 안 되는 문제 (패턴 수정 부분에 coin cell 추가 및 소숫점 전류 입력 가능)
    음극 코인셀 전압 확인을 위해 전압용 Y축 설정 추가
    개별 그래프에서 그래프 색 동일하게 나오는 부분 수정
    승인 수명에서 가장 짧은 열 기준으로 결과 나오는 부분 수정
250704
    용량 지정 시 사이클 안 나오는 오류 수정
250707
    15F Toyo, PNE 충방전기 IP 변경 반영
    충방전기 전체 확인용 메뉴 추가
250721
    코인셀 사이클에서 데이터 용량 산정 안되는 문제 수정
    cyclename 없을 때 범례 안 나오는 문제 수정
250722
    파일 선택 안 했을 경우 오류 수정
    dQdV의 축바꾸기 기능 추가
250723
    패턴 부분 정렬하여 프로그램과 일치
250724
    윈도우11 기준 UI 아래부분 짤리는 부분 고려하여 UI 사이즈 축소
250728
    Toyo 충전 용량 산정 오류 수정
250805
    Pattern 충방전 확인 반영
250819
    PyQt6로 변경, RSS rest time 변경 반영
'''
# python 3.12.7 기반
# 패키지 불러오기
'''
...
def pne_cycle_data(raw_file_path, mincapacity, ini_crate, chkir, chkir2, mkdcir):
    ...
    if (raw_file_path[-4:-1]) != "ter":
        ...
        if os.path.isdir(raw_file_path + "\\Restore\\"):
            ...
            for files in subfile:
                if "SaveEndData.csv" in files:
                    if os.stat(raw_file_path + "\\Restore\\" + files).st_size != 0 and mincapacity != None:
                        Cycleraw = pd.read_csv(raw_file_path + "\\Restore\\" + files, sep=",", skiprows=0, engine="c",
                                               header=None, encoding="cp949", on_bad_lines='skip')
                        ...
                        # 1s pulse, RSS DCIR
                            ...
                            # dcirtemp2 - 1s DCIR을 위한 pulse CCV 선정
                            dcirtemp2 = Cycleraw[(Cycleraw["steptime"] == 100) & (Cycleraw["EndState"] == 64)]
                            # dcirtemp3 - pulse 후 rest 전압 산정 (RSS OCV)
                            dcirtemp3 = Cycleraw.loc[(Cycleraw['steptime'].isin([90000, 180000, 186000, 546000])) 
                                                     & (Cycleraw['EndState'] == 64) & (Cycleraw['Condition'] == 3)]
                        ...
                        Cycleraw["OriCycle"] = Cycleraw["TotlCycle"]
                        Cycleraw = Cycleraw[(Cycleraw["Condition"] == 8)]
                        Cycleraw.index = Cycleraw["TotlCycle"]
                        Cycleraw.chgCap2 = Cycleraw.chgCap.shift(periods=-1)
                        Eff = Cycleraw.DchgCap/Cycleraw.chgCap
                        Eff2 = Cycleraw.chgCap2/Cycleraw.DchgCap
                        DchgEng = Cycleraw.DchgEngD/1000
                        Dchg = Cycleraw.DchgCap/mincapacity/1000
                        Chg = Cycleraw.chgCap/mincapacity/1000
                        Ocv = Cycleraw.Ocv/1000000
                        AvgV = Cycleraw.AvgV/1000000
                        Temp = Cycleraw.Temp/1000
                        OriCycle = Cycleraw["OriCycle"]
                        if chkir and len(OriCycle) == len(dcir):
                            df.NewData = pd.DataFrame({"Dchg": Dchg, "RndV": Ocv, "Eff": Eff, "Chg": Chg,
                                                       "DchgEng": DchgEng, "Eff2": Eff2, "dcir": dcir, "Temp": Temp,
                                                       "AvgV": AvgV, "OriCyc": OriCycle})
                        if chkir:
                            df.NewData = pd.DataFrame({"Dchg": Dchg, "RndV": Ocv, "Eff": Eff, "Chg": Chg,
                                                       "DchgEng": DchgEng, "Eff2": Eff2, "Temp": Temp, "AvgV": AvgV,
                                                       "OriCyc": OriCycle})
                            df.NewData.loc[0, "dcir"] = 0
...
def pne_Profile_continue_data(raw_file_path, inicycle, endcycle, mincapacity, inirate):
...
def pne_dcir_Profile_data(raw_file_path, inicycle, endcycle, mincapacity, inirate):
    ...
    if (raw_file_path[-4:-1]) != "ter":
        ...
        if hasattr(pneProfile, 'Profileraw'):
            ...
            # 율별 pulse 전류 확인
            dcir_curr = [(dcir_base.loc[i, "Current[mA]"] / 1000000000) for i in range(0,4)]
            # 율별 pulse C-rate 확인
            dcir_crate = [((dcir_base.loc[i, "Current[mA]"] / 1000)/mincapacity).round(2) for i in range(0,4)]
            dcir_curr.sort()
            dcir_crate.sort()
            # DCIR 시간을 0.2초로 변경
            dcir_time = [0.0, 0.2, 1.0, 10.0, 20.0]
            # Profile 데이터를 기준으로 산정 
                ...
                for time in dcir_time:
                    ...
                    if time != 0:
                        for row in range(len(profile_pvt)):
                            pvt_row = profile_pvt.iloc[row].tolist()
                            # slope를 DCIR로 산정
                            slope = linregress(dcir_curr[:len(pvt_row)], pvt_row)[0]
                            if slope > 0:
                                # slope를 DCIR로 산정
                                temp_dcir_slope.append(slope)
                            if time == dcir_time[1]:
                                # 절편을 OCV로 산정
                                ocv = linregress(dcir_curr[:len(pvt_row)],pvt_row)[1]
...
class Ui_sitool(object):
    def setupUi(self, sitool):
        ...
        self.horizontalLayout_112 = QtWidgets.QHBoxLayout(self.CycTab)
        ...
        self.chk_cyclepath.setMinimumSize(QtCore.QSize(502, 30))
        self.chk_cyclepath.setMaximumSize(QtCore.QSize(502, 30))
        font = QtGui.QFont()
        font.setFamily("malgun gothic")
        font.setPointSize(9)
        font.setBold(False)
        ...
        self.verticalLayout_16.addWidget(self.chk_cyclepath)
        ...
        self.stepnum_2.setMinimumSize(QtCore.QSize(270, 70))
        self.stepnum_2.setMaximumSize(QtCore.QSize(270, 70))
        font = QtGui.QFont()
        font.setFamily("malgun gothic")
        font.setPointSize(9)
        font.setBold(False)
        ...
        self.cycle_tab_reset.setMinimumSize(QtCore.QSize(215, 70))
        self.cycle_tab_reset.setMaximumSize(QtCore.QSize(215, 70))
        font = QtGui.QFont()
        font.setFamily("malgun gothic")
        ...
        self.verticalLayout_16.addLayout(self.horizontalLayout_119)
        ...
        self.stepnumlb.setMinimumSize(QtCore.QSize(0, 40))
        self.stepnumlb.setMaximumSize(QtCore.QSize(180, 40))
        font = QtGui.QFont()
        font.setFamily("malgun gothic")
        font.setPointSize(9)
        font.setBold(False)
        ...
        self.stepnum.setMinimumSize(QtCore.QSize(0, 40))
        self.stepnum.setMaximumSize(QtCore.QSize(300, 40))
        font = QtGui.QFont()
        font.setFamily("malgun gothic")
        font.setPointSize(9)
        font.setBold(False)
        ...
        self.horizontalLayout_108.addLayout(self.verticalLayout_6)
        ...
        self.horizontalLayout_108.addWidget(self.cycle_tab)
        self.horizontalLayout_112.addLayout(self.horizontalLayout_108)
        ...
        self.verticalLayout_13 = QtWidgets.QVBoxLayout(self.SetTab)
        ...
        self.Capacitynum.setMinimumSize(QtCore.QSize(320, 100))
        ...
        self.verticalLayout_29.addLayout(self.horizontalLayout_124)
        ...
        self.verticalLayout_20.addLayout(self.horizontalLayout_125)
        ...
        self.verticalLayout_20.addLayout(self.horizontalLayout_126)
        ...
        self.verticalLayout_20.addLayout(self.horizontalLayout_127)
        self.horizontalLayout_130 = QtWidgets.QHBoxLayout()
        self.horizontalLayout_130.setObjectName("horizontalLayout_130")
        self.label_5 = QtWidgets.QLabel(parent=self.SetTab)
        self.label_5.setMinimumSize(QtCore.QSize(320, 70))
        self.label_5.setMaximumSize(QtCore.QSize(320, 70))
        font = QtGui.QFont()
        font.setFamily("malgun gothic")
        font.setPointSize(10)
        font.setBold(False)
        ...
        self.horizontalLayout_130.addWidget(self.label_5)
        spacerItem13 = QtWidgets.QSpacerItem(326, 70, QtWidgets.QSizePolicy.Policy.Expanding, QtWidgets.QSizePolicy.Policy.Minimum)
        self.horizontalLayout_130.addItem(spacerItem13)
        self.verticalLayout_20.addLayout(self.horizontalLayout_130)
        ...
        self.verticalLayout_20.addLayout(self.horizontalLayout_131)
        ...
        self.verticalLayout_20.addLayout(self.horizontalLayout_132)
        self.verticalLayout_29.addLayout(self.verticalLayout_20)
        self.horizontalLayout_136.addLayout(self.verticalLayout_29)
        ...
        self.horizontalLayout_136.addWidget(self.set_tab)
        ...
        self.horizontalLayout_155 = QtWidgets.QHBoxLayout(self.dvdq)
        ...
        self.horizontalLayout_152.addWidget(self.mat_dvdq_btn)
        ...
        self.horizontalLayout_152.addWidget(self.dvdq_fitting)
        self.verticalLayout_15.addLayout(self.horizontalLayout_152)
        ...
        self.horizontalLayout_153.addWidget(self.pro_dvdq_btn)
        ...
        self.horizontalLayout_153.addWidget(self.dvdq_fitting_2)
        self.verticalLayout_15.addLayout(self.horizontalLayout_153)
        self.verticalLayout_22.addLayout(self.verticalLayout_15)
        self.horizontalLayout_154.addLayout(self.verticalLayout_22)
        ...
        self.horizontalLayout_154.addWidget(self.dvdq_simul_tab)
        ...
        self.load_cycparameter.setMinimumSize(QtCore.QSize(335, 80))
        self.load_cycparameter.setMaximumSize(QtCore.QSize(335, 80))
        font = QtGui.QFont()
        font.setFamily("malgun gothic")
        font.setPointSize(12)
        font.setBold(True)
        ...
        self.AppCycleTabReset.setMinimumSize(QtCore.QSize(313, 80))
        self.AppCycleTabReset.setMaximumSize(QtCore.QSize(313, 80))
        font = QtGui.QFont()
        font.setFamily("malgun gothic")
        font.setPointSize(12)
        font.setBold(True)
        ...
        self.pathappcycestimation.setMinimumSize(QtCore.QSize(313, 80))
        self.pathappcycestimation.setMaximumSize(QtCore.QSize(313, 80))
        font = QtGui.QFont()
        font.setFamily("malgun gothic")
        font.setPointSize(12)
        font.setBold(True)
        ...
        self.folderappcycestimation.setMinimumSize(QtCore.QSize(313, 80))
        self.folderappcycestimation.setMaximumSize(QtCore.QSize(313, 80))
        font = QtGui.QFont()
        font.setFamily("malgun gothic")
        font.setPointSize(12)
        font.setBold(True)
        ...
        self.txt_storageratio.setMinimumSize(QtCore.QSize(20, 20))
        self.txt_storageratio.setMaximumSize(QtCore.QSize(20, 20))
        font = QtGui.QFont()
        font.setFamily("malgun gothic")
        ...
        self.txt_storageratio2.setMinimumSize(QtCore.QSize(20, 20))
        self.txt_storageratio2.setMaximumSize(QtCore.QSize(20, 20))
        font = QtGui.QFont()
        font.setFamily("malgun gothic")
        ...
        self.SimGroupConst.setMinimumSize(QtCore.QSize(620, 240))
        ...
        spacerItem15 = QtWidgets.QSpacerItem(143, 44, QtWidgets.QSizePolicy.Policy.Minimum, QtWidgets.QSizePolicy.Policy.Expanding)
        self.verticalLayout_26.addItem(spacerItem15)
        ...
        spacerItem16 = QtWidgets.QSpacerItem(142, 44, QtWidgets.QSizePolicy.Policy.Minimum, QtWidgets.QSizePolicy.Policy.Expanding)
        ...
        self.line_2.setMinimumSize(QtCore.QSize(598, 3))
    ...
    def retranslateUi(self, sitool):
        ...
        sitool.setWindowTitle(_translate("sitool", "BatteryDataTool v250819"))
...
"3) 첫 Cycle의 Crate 기준"))
        ...
        self.mkdcir.setText(_translate("sitool", "PNE RSS DCIR (SOC 30/ 50/ 70 충전, SOC 70/ 50/ 30 방전, 1s Pulse/RSS)\n"
"PNE RSS DCIR2 (SOC 70/ 50/ 30/ 15 방전, 1s Pulse/RSS) - 그래프는 방전 70%"))
...
"예) 3 4 5 8-9"))
        ...
        self.ContinueConfirm.setText(_translate("sitool", "HPPC/ GITT"))
...
" (0, 0이면 최소, 최대 기준)"))
        ...
        self.label_5.setText(_translate("sitool", "Battery Status 결과 확인"))
...
class WindowClass(QtWidgets.QMainWindow, Ui_sitool):
                                ...
                                if hasattr(temp[1], "stepchg"):
                                    if len(temp[1].stepchg) > 2:
                                        ...
                                        if self.ect_saveok.isChecked() and save_file_name:
                                            ...
                                            continue_df.to_csv(save_file_name, index=False, sep=',',
                                                                header=["time(s)",
                                                                        "Voltage(V)",
                                                                        "Current(A)",
                                                                        "Temp."])
                    ...
                    for FolderBase in subfolder:
                        ...
                        if "Pattern" not in FolderBase:
                            ...
                            for CycNo in CycleNo:
                                cyccountmax = len(CycleNo)
                                cyccount = cyccount + 1
                                # progressdata = (foldercount + (chnlcount + cyccount/cyccountmax - 1)/chnlcountmax - 1)/foldercountmax * 100
                                # progressdata = progress(foldercount, foldercountmax, cyccount, cyccountmax, chnlcount, chnlcountmax)
                                progressdata = progress(foldercount, foldercountmax, chnlcount, chnlcountmax, cyccount, cyccountmax)
                                # progressdata = (
                                #     (
    ...
    def toyo_base_data_make(self, toyo_num, blkname):
        ...
        if os.path.isfile(toyoworkpath):
            ...
            # toyo_data["day"] = toyo_data['testname'].str.split("_").str[0]
            # toyo_data["part"] = toyo_data['testname'].str.split("_").str[2] + "파트"
            # toyo_data["name"] = toyo_data['testname'].str.split("_").str[3]
            # toyo_data["day"] = toyo_data['testname'].str.split(" ").str[0]
            # toyo_data["part"] = toyo_data['testname'].str.split(" ").str[1]
            # toyo_data["name"] = toyo_data['testname'].str.split(" ").str[2]
    ...
    def toyo_table_make(self, num_i, num_j, toyo_num, blkname):
        ...
        for i in range(1, num_i + 1):
            for j in range(1, num_j + 1):
                ...
                if (str(self.FindText.text()) != "") and str(self.FindText.text()) in self.df.loc[i + (j - 1) * num_i,"testname"]:
                    self.tb_channel.item(j - 1, i - 1).setFont(QtGui.QFont("Malgun gothic", 10, QtGui.QFont.Weight.Bold))
    ...
    def pne_data_make(self, pne_num, blkname):
        ...
        if os.path.isfile(pneworkpath):
            ...
            # self.df["day"] = self.df['testname'].str.split(" ").str[0]
            # self.df["part"] = self.df['testname'].str.split(" ").str[1]
            # self.df["name"] = self.df['testname'].str.split(" ").str[2]
            # self.df["day"] = self.df['testname'].str.split("_").str[0]
            # self.df["part"] = self.df['testname'].str.split("_").str[2] + "파트"
            # self.df["name"] = self.df['testname'].str.split("_").str[3]
    ...
    def tb_room_combobox(self):
        if self.tb_room.currentIndex() == 0:
            ...
            self.tb_cycler.clear()
            self.tb_cycler.addItems(["PNE17", "PNE18", "PNE19", "PNE20", "PNE21"])
    ...
    def set_log_confirm_button(self):
        ...
        #Short Profile 확인용
            graph_set_profile(Profile.Time, Profile.Vol, ax1, 3.0, 4.8, 0.2, "Time(hr)", "Voltage (V)", "CCV", 1, 0, 0, 0)
            graph_set_profile(Profile.Time, Profile.ocv, ax1, 3.0, 4.8, 0.2, "Time(hr)", "Voltage (V)", "OCV", 2, 0, 0, 0)
            graph_set_profile(Profile.Time, Profile.Curr, ax2, -5, 6, 1, "Time(hr)", "Curr", "Curr", 1, 0, 0, 0)
            graph_set_profile(Profile.Time, Profile.SOC, ax3, 0, 110, 10, "Time(hr)", "SOC", "SOC", 1, 0, 0, 0)
            graph_set_profile(Profile.Time, Profile.SOCraw, ax3, 0, 110, 10, "Time(hr)", "SOCraw", "rawSOC", 2, 0, 0, 0)
            graph_set_profile(Profile.Time, Profile.T_bat, ax4, 20, 50, 4, "Time(hr)", "Temperature(℃)", "T_bat", 1, 0, 0, 0)
            graph_set_profile(Profile.Time, Profile.T_usb, ax4, 20, 50, 4, "Time(hr)", "temp.", "T_usb", 2, 0, 0, 0)
            graph_set_profile(Profile.Time, Profile.T_chg, ax4, 20, 50, 4, "Time(hr)", "temp.", "T_chg", 3, 0, 0, 0)
            graph_set_profile(Profile.Time, Profile.T_lrp, ax4, 20, 50, 4, "Time(hr)", "temp.", "T_lrp", 4, 0, 0, 0)
            graph_set_profile(Profile.Time, Profile.cap_max, ax5, 95, 101, 1, "Time(hr)", "cap_max", "cap_max", 1, 0, 0, 0)
        # Short 관련
    ...
    def set_confirm_button(self):
        ...
        if datafilepath:
            ...
            for filepath in datafilepath:
                ...
                namelist = filepath.split("/")
                for i in cyclecountmax:
                    temp = set_battery_status_log_Profile(chkcyc[2].Profile, mincapa, i, chkcyc.set)
                    # progressdata = ((filecount - 1) + (i - cyclecountmax[0] + 1)/len(
                    #     cyclecountmax))/filecountmax * 100
                    progressdata = progress(filecount, filecountmax, (cyclecountmax[0] - i), cyclecountmax, 1, 1)
                    self.progressBar.setValue(int(progressdata))
                    if hasattr(temp, "ChgProfile"):
                        chgrealcap = str(round(temp.ChgProfile.SOC2.max(), 2))
                        chgmaxtime = str(round(temp.ChgProfile.Time.max() * 60, 1))
                        caplegend = "{:3}".format(str(i)) + " C:" + "{:8}".format(chgrealcap)
                        temp.ChgProfile = temp.ChgProfile[(temp.ChgProfile["Time"] < 4)]
                        graph_set_profile(temp.ChgProfile.Time, temp.ChgProfile.Vol, ax1, 3.0, 4.8, 0.2, "Time(hr)", "Voltage (V)", "", 0, 0, 4, 1)
                        graph_set_profile(temp.ChgProfile.Time, temp.ChgProfile.Curr, ax3, 0, 2.4, 0.2, "Time(hr)", "Curr", "", 0, 0, 4, 1)
                        graph_set_profile(temp.ChgProfile.Time, temp.ChgProfile.Temp, ax5, 20, 50, 4, "Time(hr)", "temp.", "", 0, 0, 4, 1)
                        graph_set_profile(temp.ChgProfile.Time, temp.ChgProfile.SOC, ax7, 0, 120, 10, "Time(hr)", "SOC", "", 0, 0, 4, 1)
                        graph_set_profile(temp.ChgProfile.Time, temp.ChgProfile.SOC2, ax9, 0, 120, 10, "Time(hr)", "real SOC", "", 0, 0, 4, 1)
                    else:
                        chgrealcap = " "
                        chgmaxtime = " "
                    if hasattr(temp, "DchgProfile"):
                        ...
                        dchgmaxtime = str(round(temp.DchgProfile.Time.max() * 60, 1))
                        caplegend = caplegend + " D:" + "{:6}".format(dchgrealcap)
                        graph_set_profile(temp.DchgProfile.Time, temp.DchgProfile.Vol, ax2, 3.0, 4.6, 0.2, "Time(hr)", "Voltage (V)", "", 0, 0, 11, 1)
                        graph_set_profile(temp.DchgProfile.Time, temp.DchgProfile.Curr, ax4, -0.6, 0.1, 0.1, "Time(hr)", "Curr", "", 0, 0, 11, 1)
                        graph_set_profile(temp.DchgProfile.Time, temp.DchgProfile.Temp, ax6, 20, 50, 4, "Time(hr)", "temp.", "", 0, 0, 11, 1)
                        graph_set_profile(temp.DchgProfile.Time, temp.DchgProfile.SOC, ax8, 0, 120, 10, "Time(hr)", "SOC", "", 0, 0, 11, 1)
                        if self.saveok.isChecked():
                            graph_set_profile(temp.DchgProfile.Time, 100 - temp.DchgProfile.SOC2, ax10, 0, 120, 10, "Time(hr)", "real SOC",
                                              i, 0, 0, 11, 1)
                        else:
                            graph_set_profile(temp.DchgProfile.Time, 100 - temp.DchgProfile.SOC2, ax10, 0, 120, 10, "Time(hr)", "real SOC",
                                              caplegend, 0, 0, 11, 1)
    ...
    def ect_data(self, datafilepath):
        ''' 250515 SBP + Sleep
        0:Time, 1:voltage_now(mV), 2:V avg (mV), 3:Ctype(Etc)-ChargCur, 4:Current Avg., 5:sbp_batt_voltage,
        6:sbp_inst_current, 7:sbp_avg_current, 8:Level, 9:Charging, 10:Battery_Cycle, 11:diffTime, 12:Temperature(BA),
        13:temp_ambient, 14:batt_temp, 15:batt_temp_adc, 16:batt_temp_raw, 17:batt_wpc_temp, 18:batt_wpc_temp_adc,
        19:chg_temp, 20:chg_temp_adc, 21:dchg_temp, 22:dchg_temp_adc, 23:usb_temp, 24:usb_temp_adc, 25:compVoltage,
        26:ectSOC, 27:RSOC, 28:SOC_RE, 29:sbp_SOC, 30:SOC_EDV, 31:SOH, 32:sbp_SOH, 33:AnodePotential, 34:SOH_dR,
        35:SOH_CA, 36:SOH_X, 37:SC_VALUE, 38:SC_SCORE, 39:SC_V_Acc, 40:SC_V_Avg,
        41:LUT_VOLT0, 42:LUT_VOLT1, 43:LUT_VOLT2, 44:LUT_VOLT3, 45:T_move'''
        Profile = pd.read_csv(datafilepath, sep=",", on_bad_lines='skip', skiprows = 1)
        ...
        if ("sbp_batt_voltage" in Profile.columns) or (len(Profile.columns) == 46):
            Profile = Profile.iloc[:, [0, 1, 2, 3, 4, 12, 8, 26, 27, 28, 9, 10, 33, 38, 2]]
            Profile.columns = ['Time', 'Voltage(mV)', 'V avg (mV)', 'Ctype(Etc)-ChargCur', 'CurrAvg', 'Temperature(BA)', 'Level', 'ECTSOCraw',
                            'ECTRSOC', 'ECTSOC', 'Charging', 'Battery_Cycle', 'anodeE', 'SC_SCORE', 'Vavg'] 
        elif ("sbp_batt_voltage" in Profile.columns) or (len(Profile.columns) == 49):
            Profile = Profile.iloc[:, [0, 1, 2, 3, 4, 12, 8, 26, 27, 28, 9, 10, 33, 38, 2]]
            Profile.columns = ['Time', 'Voltage(mV)', 'V avg (mV)', 'Ctype(Etc)-ChargCur', 'CurrAvg', 'Temperature(BA)', 'Level', 'ECTSOCraw',
                            'ECTRSOC', 'ECTSOC', 'Charging', 'Battery_Cycle', 'anodeE', 'SC_SCORE', 'Vavg'] 
        else:
            if "LUT_VOLT0" in Profile.columns:
                Profile = Profile.iloc[:, [0, 1, 3, 4, 12, 5, 23, 24, 25, 6, 7, 28, 33, 36, 37, 38, 39, 2]]
                Profile.columns = ['Time', 'Voltage(mV)', 'Ctype(Etc)-ChargCur', 'CurrAvg', 'Temperature(BA)', 'Level', 'ECTSOCraw',
                                'ECTRSOC', 'ECTSOC', 'Charging', 'Battery_Cycle', 'anodeE', 'SC_SCORE', 
                                '1stepV', '2stepV', '3stepV', '4stepV', 'Vavg']
            else:
                Profile = Profile.iloc[:, [0, 1, 3, 4, 9, 5, 23, 24, 25, 6, 7, 28, 33, 2]]
                Profile.columns = ['Time', 'Voltage(mV)', 'Ctype(Etc)-ChargCur', 'CurrAvg', 'Temperature(BA)', 'Level', 'ECTSOCraw',
                                'ECTRSOC', 'ECTSOC', 'Charging', 'Battery_Cycle', 'anodeE', 'SC_SCORE', 'Vavg'] 
        Profile.Time = '20'+ Profile['Time'].astype(str)
        Profile["Charging"] = Profile["Charging"].str.replace(" ","")
        Profile["PlugType"] = Profile["Charging"]
        Profile["PlugType"] = Profile["PlugType"].str.replace("Charging", "AC")
        Profile["PlugType"] = Profile["PlugType"].str.replace("Full", "AC")
        Profile["PlugType"] = Profile["PlugType"].str.replace("Discharging", "Unplugged")
        if "1stepV" in Profile.columns:
            Profile = Profile[["Time", "Level", "ECTSOCraw", "ECTRSOC", "ECTSOC", "Voltage(mV)", "Ctype(Etc)-ChargCur",
                                "CurrAvg", "Charging", "Temperature(BA)", "Battery_Cycle", "PlugType", "anodeE", 'SC_SCORE',
                                '1stepV', '2stepV', '3stepV', '4stepV', 'Vavg']]
        else:
            Profile = Profile[["Time", "Level", "ECTSOCraw", "ECTRSOC", "ECTSOC", "Voltage(mV)", "Ctype(Etc)-ChargCur",
                                "CurrAvg", "Charging", "Temperature(BA)", "Battery_Cycle", "PlugType", "anodeE", 'SC_SCORE', 'Vavg']]
        Profile = Profile[:-1]
        cycmin = int(Profile.Battery_Cycle.min())
        cycmax = int(Profile.Battery_Cycle.max())
        if self.realcyc.isChecked() == 0:
            if cycmin != cycmax:
                n = cycmin
                Profile['Battery_Cycle'] = n
                for m in range(1, len(Profile) - 1):
                    if (Profile.loc[m, 'PlugType'] == "Unplugged" or Profile.loc[m, 'PlugType'] == " NONE") and (
                            Profile.loc[m + 1, 'PlugType'] == "AC" or Profile.loc[m + 1, 'PlugType'] == " PDIC_APDO"):
                        n = n + 1
                        Profile.loc[m + 1, 'Battery_Cycle'] = n
                    Profile.loc[m + 1, 'Battery_Cycle'] = n
        cycmax = int(Profile.Battery_Cycle.max())
        if "1stepV" in Profile.columns:
            Profile.columns = ["Time", "SOC", "SOCectraw", "RSOCect", "SOCect", "Vol", "Curr", "CurrAvg", "Type", "Temp",
                                "Cyc", "State", "anodeE", 'short', '1stepV', '2stepV', '3stepV', '4stepV', 'Vavg']
        else:
            Profile.columns = ["Time", "SOC", "SOCectraw", "RSOCect", "SOCect", "Vol", "Curr", "CurrAvg", "Type", "Temp",
                                "Cyc", "State", "anodeE", 'short', 'Vavg']
        if not Profile.empty:
            # 시간 확인
            Profile["Time"] = pd.to_datetime(Profile["Time"], format="%Y%m%d %H:%M:%S.%f")
            Profile["Time"] = Profile["Time"] - Profile["Time"].loc[0]
            Profile["Time"] = Profile["Time"].dt.total_seconds().div(3600).astype(float)
            Profile['Curr']=Profile['Curr'].apply(float)
            Profile['CurrAvg']=Profile['CurrAvg'].apply(float)
            Profile['SOC']=Profile['SOC'].apply(float)
            Profile['Vol']=Profile['Vol'].apply(float)/1000
    ...
    def ect_short_button(self):
        ...
        if datafilepaths:
            ...
            # 최근 사이클 산정 및 전체 사이클 적용여부 확인
                ...
                canvas = FigureCanvas(fig)
                toolbar = NavigationToolbar(canvas, None)
                Profile = self.ect_data(datafilepath)
            #Short Profile 확인용
                graph_set_profile(Profile.Time, Profile.Vol, ax1, 3.0, 4.8, 0.2, "Time(hr)", "Voltage (V)", "", 0, 0, 0, 0)
                # graph_set(Profile.Time, Profile.anodeE, ax2, -0.1, 0.8, 0.1, "Time(hr)", "anodeE", "", 99)
                graph_set_profile(Profile.Time, Profile.Curr, ax2, -0.6, 2.4, 0.2, "Time(hr)", "Curr", "", 0, 0, 0, 0)
                graph_set_profile(Profile.Time, Profile.Temp, ax3, 20, 50, 4, "Time(hr)", "temp.", "", 0, 0, 0, 0)
                graph_set_profile(Profile.Time, Profile.SOC, ax4, 0, 120, 10, "Time(hr)", "SOC", "", 0, 0, 0, 0)
                graph_set_profile(Profile.Time, Profile.SOCect, ax4, 0, 120, 10, "Time(hr)", "SOCect", "", 1, 0, 0, 0)
            # Short 관련
                graph_set_profile(Profile.Time, Profile.short, ax5, 0, 6, 1, "Time(hr)", "Short_Score", "", 0, 0, 0, 0)
                Chgnamelist = datafilepath.split("/")
                tab_layout.addWidget(toolbar)
                tab_layout.addWidget(canvas)
                self.set_tab.addTab(tab, Chgnamelist[-1])
        ...
        if datafilepaths:
            ...
            # 최근 사이클 산정 및 전체 사이클 적용여부 확인
                ...
                if self.saveok.isChecked():
                    ...
                    dfchg = pd.DataFrame()
                    dfdchg = pd.DataFrame()
                fig, ((ax1, ax5), (ax2, ax6), (ax3, ax7), (ax4, ax8)) = plt.subplots(nrows=4, ncols=2, figsize=(8, 10))
                tab = QtWidgets.QWidget()
                tab_layout = QtWidgets.QVBoxLayout(tab)
                canvas = FigureCanvas(fig)
                toolbar = NavigationToolbar(canvas, None)
            ...
            # 전체 사이클과 최근 사이클 기준 설정
                ...
                for i in cyclecountmax:
                    ...
                    if DchgRealAvgCap.max() > 1:
                        self.socmaxcapacity.setText(str(int(DchgRealAvgCap.max())))
                    ...
                    progressdata = (i - cyclecountmax[0] + 1)/len(cyclecountmax) * 100
                    self.progressBar.setValue(int(progressdata))
                ...
                # 방전 관련
                    graph_soc_set(DchgProfile.Time, DchgProfile.Vol, ax1, 3.0, 4.8, 0.2, "Time(hr)", "Voltage (V)", "", 1)
                    graph_soc_set(DchgProfile.Time, DchgProfile.Vavg_est, ax1, 3.0, 4.8, 0.2, "Time(hr)", "Voltage (V)", "", 3)
                    graph_soc_set(DchgProfile.Time, DchgProfile.anodeE, ax2, 0, 0.8, 0.1, "Time(hr)", "Anode Voltage (V)", "", 1)
                    graph_soc_set(DchgProfile.Time, DchgProfile.CurrAvg, ax3, 0, 0, 0, "Time(hr)", "Current Average Rate", "", 1)
                    graph_soc_set(DchgProfile.Time, DchgProfile.Temp, ax4, -20, 60, 10, "Time(hr)", "Temperature (℃)", "", 1)
                # SOC 비교
                    graph_soc_set(DchgProfile.Time, DchgProfile.SOCectraw, ax5, 0, 110, 10, "Time(hr)", "ASOC_ect", "ASOC_ect", 6)
                    graph_soc_set(DchgProfile.Time, DchgProfile.RSOCect, ax5, 0, 110, 10, "Time(hr)", "SOCraw_ect", "SOCraw_ect", 7)
                    graph_soc_set(DchgProfile.Time, DchgProfile.SOCect, ax5, 0, 110, 10, "Time(hr)", "SOC_ect", "SOC_ect", 4)
                    graph_soc_set(DchgProfile.Time, DchgProfile.SOC, ax6, 0, 110, 10, "Time(hr)", "SOC/ SOC_ect/ SOC_ref", "SOC", 3)
                    graph_soc_set(DchgProfile.Time, DchgProfile.SOCect, ax6, 0, 110, 10, "Time(hr)", "SOC/ SOC_ect/ SOC_ref", "SOC_ect", 4)
                    graph_soc_set(DchgProfile.Time, DchgProfile.SOCrefAvg, ax6, 0, 110, 10, "Time(hr)", "SOC/SOCect/SOCref", "SOC_ref", 5)
                    graph_soc_set(DchgProfile.Time, DchgProfile.SOCError, ax7, -5, 6, 1, "Time(hr)", "Error(%)", "SOC", 3)
                    graph_soc_set(DchgProfile.Time, DchgProfile.SOCectError, ax7, -5, 6, 1, "Time(hr)", "Error(%)", "SOC_ect", 4)
                    graph_soc_err(DchgProfile.SOCrefAvg, DchgProfile.SOCError, ax8, -5, 6, 1, "SOCref", "Error(%)", "SOC", 3)
                    graph_soc_err(DchgProfile.SOCrefAvg, DchgProfile.SOCectError, ax8, -5, 6, 1, "SOCref", "Error(%)", "SOC_ect", 4)
                if self.saveok.isChecked() and save_file_name:
    ...
    def ect_set_profile_button(self):
        ...
        if datafilepaths:
            ...
            # 최근 20 cycle 기준으로 설정
                ...
                for i in cyclecountmax:
                    DchgProfile = tempDchgProfile[(tempDchgProfile.Cyc == i)]
                    ChgProfile = tempChgProfile[(tempChgProfile.Cyc == i)]
                    DchgProfile = DchgProfile.reset_index()
                    ...
                    DchgProfile.SOCectError = DchgProfile.SOCrefAvg - DchgProfile.SOCect
                # 충전 관련
                    graph_set_profile(ChgProfile.Time, ChgProfile.Vol, ax1, 3.0, 4.8, 0.2, "Time(hr)", "Voltage (V)", "", 0, 0, 4, 1)
                    graph_set_profile(ChgProfile.Time, ChgProfile.anodeE, ax3, -0.1, 0.3, 0.05, "Time(hr)", "anodeE (V)", "", 0, 0, 4, 1)
                    graph_set_profile(ChgProfile.Time, ChgProfile.CurrAvg/1000, ax5, 0, 0, 0, "Time(hr)", "CurrAvg (A)", "", 0, 0, 4, 1)
                    graph_set_profile(ChgProfile.Time, ChgProfile.Temp, ax7, 20, 50, 4, "Time(hr)", "temp.(℃ )", "", 0, 0, 4, 1)
                    graph_set_profile(ChgProfile.Time, ChgProfile.SOC, ax9, 0, 120, 10, "Time(hr)", "SOC", "", 1, 0, 4, 1)
                    graph_set_profile(ChgProfile.Time, ChgProfile.SOCect, ax9, 0, 120, 10, "Time(hr)", "SOC", "", 2, 0, 4, 1)
                # 방전 관련
                    graph_set_profile(DchgProfile.Time, DchgProfile.Vol, ax2, 3.0, 4.8, 0.2, "Time(hr)", "Voltage (V)", "", 0, 0, 11, 1)
                    graph_set_profile(DchgProfile.Time, DchgProfile.anodeE, ax4, 0, 0.8, 0.1, "Time(hr)", "anodeE (V)", "", 0, 0, 11, 1)
                    graph_set_profile(DchgProfile.Time, DchgProfile.CurrAvg/1000, ax6, 0, 0, 0, "Time(hr)", "CurrAvg (A)", "", 0, 0, 11, 1)
                    graph_set_profile(DchgProfile.Time, DchgProfile.Temp, ax8, 20, 50, 4, "Time(hr)", "temp.(℃)", "", 0, 0, 11, 1)
                    graph_set_profile(DchgProfile.Time, DchgProfile.SOC, ax10, 0, 120, 10, "Time(hr)", "SOC", "", 1, 0, 11, 1)
                    graph_set_profile(DchgProfile.Time, DchgProfile.SOCect, ax10, 0, 120, 10, "Time(hr)", "SOC", "", 2, 0, 11, 1)
                # SOC 비교
                    if "1stepV" in ChgProfile.columns:
                        graph_set_guide(ChgProfile.Time, ChgProfile["1stepV"], ax1, 3.0, 4.8, 0.2, "Time(hr)", "Voltage (V)", "", 0, 4, 1)
                        graph_set_guide(ChgProfile.Time, ChgProfile["2stepV"], ax1, 3.0, 4.8, 0.2, "Time(hr)", "Voltage (V)", "", 0, 4, 1)
                        graph_set_guide(ChgProfile.Time, ChgProfile["3stepV"], ax1, 3.0, 4.8, 0.2, "Time(hr)", "Voltage (V)", "", 0, 4, 1)
                        graph_set_guide(ChgProfile.Time, ChgProfile["4stepV"], ax1, 3.0, 4.8, 0.2, "Time(hr)", "Voltage (V)", "", 0, 4, 1)
                if self.saveok.isChecked() and save_file_name:
                    dfdchg = dfdchg._append(DchgProfile)
    ...
    def simulation_confirm_button(self):
        ...
        for input_data_path in all_input_data_path:
            ...
            for i in range(0, 100000):
                ...
                # 사이클 Fd 구하기 (타펠식, 아레니우스식의 복합식 사용) - 용량/저항
                if "df_cyc" in locals():
                    ...
                    return
                if "df_stg" in locals():
                    # 저장1 Fd 구하기 (Tafel식 사용 delta E = a ln i) - 용량/저항
                    ...
                    # 저장2 Fd 구하기 (Tafel식 사용 delta E = a ln i) - 용량/저항
